...
def get_random_int(mini_num,max_num):
    ran_int=random.randint(mini_num,max_num)
    return ran_int
...
def get_guess(max_value):
    ...
    if user_guess.isdigit()==False or int(user_guess)<1 or int(user_guess)>max_value:
        return -1
    else:
        if int(user_guess)==get_random_int(1,max_value):
            return True
        else:
            return False
...
def play_game():
    ...
    while run_times<4:
        ...
        run_times+=1
    correct_percent=correct_times*100//4
    if whether_invalid==0:
        print("You guessed "+str(correct_percent)+"% of the random numbers correctly.")
    else:
        None
